...
if (!admin.apps.length) {
    try {
        ...
        console.log('Firebase initialized successfully in Remote Wipe');
    } catch (error) {
        console.error("KRITIS: Gagal memuat konfigurasi Firebase di Remote Wipe.", error);
    }
}
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (!isMatch) {
            await catatLog('REMOTE_WIPE_FAIL', { 
                userEmail: email, 
                ipAddress: req.ip || req.headers['x-forwarded-for'] || 'unknown', 
                reason: 'Wrong destruction code' 
            });
            return res.status(403).json({ message: 'Email atau Kode Penghancur salah.' });
        }
        ...
        await catatLog('REMOTE_WIPE_SUCCESS', { 
            userEmail: email, 
            ipAddress: req.ip || req.headers['x-forwarded-for'] || 'unknown' 
        });
